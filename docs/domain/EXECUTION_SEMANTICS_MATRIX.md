...
| Hip rafter H1                  | Section, physical target/support faces                             | Not applicable                             | Compound termination, backing/drop and connector are selected operations                                               | Ridge/apex, eaves/plate, intersecting jacks                                           | Regular equal-pitch centerline/face length, conceptual double-cheek cut, backing angle. **V39** makes backing/drop an explicit `HipExecutionIntent`, and proves the vertical side face J1 terminates against (unaffected by either operation) | **PARTIALLY MODELED**                                                | Hip top surface for the selected backing/drop, bearing/contact, hardware, blank; **REQUIRES STRUCTURAL DESIGN** | `timber-model` + `roof-math`                                                          | T1, T4   |
| Jack rafter J1                 | Jack/H1 sections and actual H1 face                                | Not applicable                             | Butt/cheek/hanger detail must be selected; no universal face deduction                                                 | Hip end and wall/eaves support                                                        | Reference: axis ends on the theoretical vertical H1 centre plane. **V39** adds, on explicit `hip-face-butt` intent, the finished square butt against the hip's near vertical side face (`hipWidthMm / √2`, then along the jack axis); both results coexist | **PARTIALLY MODELED** — one selected connection now resolves physically | Hardware/hanger variants, machining allowance, procurement blank; **REQUIRES STRUCTURAL DESIGN**      | `timber-model` + `roof-math`; connector snapshot when applicable                      | T4, T5   |
...
### V39 status of slice 5

The first discriminated slice landed: `JackToHipConnection = 'hip-face-butt'`
resolves a physical finished J1 end from an explicit intent, keeps the reference
geometry beside it, and reports `executionStatus: 'fabrication-resolved'`. It
deliberately stops short of procurement — `allowance: 'not-included'`, so no
blank requirement is created (ADR-009/ADR-010). Hardware variants, the hip top
surface for a chosen backing/drop, and H1's own blank remain unmodelled. See
