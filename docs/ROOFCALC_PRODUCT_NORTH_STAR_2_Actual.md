...
## 16. Advanced 3D / BIM / IFC direction

The long-term visualization target is not merely a decorative 3D preview. RoofCalc should evolve toward a **technical roof CAD/BIM workbench for carpenters and roofers** in which one canonical project connects 2D/3D geometry, fabrication, roof build-up, coverings, quantities, costing and execution documentation.

The critical invariant is:

```text
canonical RoofCalc project
        ↓
existing geometry / fabrication / covering solvers
        ↓
renderer-neutral technical scene
      ↙        ↘
     2D        3D
      ↘        ↙
 schedules / details / PDF
```

The 3D layer must never become a second calculation engine. A renderer may create meshes from already-resolved technical geometry, but it must not re-derive roof formulas, cuts, placements or material rules from those meshes.

The preferred web stack to benchmark first is **Three.js + React Three Fiber**, isolated in the presentation layer above a renderer-neutral scene contract. Camera, selection, clipping, exploded-view amount, visibility filters and viewport state remain transient workbench state.

For BIM/IFC, research **That Open Engine / Fragments** first as a candidate reference-model and semantic IFC layer. Benchmark it against xeokit and Autodesk Platform Services before freezing a dependency. Imported IFC should initially remain a separate reference scene; conversion into editable RoofCalc geometry must be explicit, validated and limited to supported semantics.

Generative 3D tools such as Meshy/Higgsfield may later assist with non-technical assets, appearance models, prototypes or marketing visuals, but must never own authoritative dimensions, structural geometry, cuts, quantities, procurement or costing.

The detailed strategic contract lives in:

`docs/ARCHITECTURE_FUTURE_3D_BIM_IFC.md`
