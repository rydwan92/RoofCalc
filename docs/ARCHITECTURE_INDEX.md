...
**This is the current-state map, after V52.** Read it after `PROJECT_BLUEPRINT.md`
...
| `roof-system-core` | **V51.** Pure roof-system requirements: structured component roles, a finite quantity-rule vocabulary, even spacing, commercial section assembly per straight run, `roof-drainage-component` / drainage-intent schemas and the drainage planner (runs, corners, end caps, hooks, outlets, downpipes). **V52**: hooks kept off gutter joints, gutter purchase policy (reuse via `procurement-core`), downpipe routes, `roof-system-component` line components (rolls, open ends, per ridge tile) and `roof-window-component` openings with structural flashing compatibility. Price-free; features arrive as structural inputs. | `procurement-core`, `zod` |
...
| `ARCHITECTURE_V52_ROOF_DETAIL_SYSTEM` | opening perimeters and ridge-line ends, hooks vs joints, gutter purchase policy, downpipe routes, ridge/eave/verge components, window flashings, System dachu workspace, checklist, V52 catalogue |
